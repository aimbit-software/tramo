 ...
 *
 * Desktop: a floating pill. A soft highlight slides under the hovered link,
 * the rest dim, and an accent dot marks the current page.
 *
...
export function AppNav({ links, workspaceName, actions }: AppNavProps) {
  ...
  return (
    <header className="sticky top-3 z-40 mx-auto mt-3 w-full max-w-5xl px-3">
      <nav
        aria-label={t("label")}
        className="flex h-14 items-center gap-3 rounded-pill bg-surface/80 pr-2 pl-5 shadow-lg shadow-black/20 backdrop-blur"
      >
        <Link href="/" className="flex-none font-display text-sm font-medium">
          {t("appName")}
        </Link>
              ...
              return (
                ...
                >
                  <Link
                    ...
                    onClick={closeSheet}
                    aria-current={active ? "page" : undefined}
                    className={`flex items-center justify-between py-3 font-display text-2xl transition-colors duration-150 ease-signature hover:text-ink motion-reduce:transition-none ${
                      active ? "text-ink" : "text-ink-muted"
                    }`}
                  >
                    {link.label}
